...
B <- raw %>% filter(!is.na(`Ct Mean`)) %>% filter(!is.na(Haplotype)) %>% filter(!is.na(`Suggest Result`)) %>% filter(!is.na(`Initial Target Copies`) & Haplotype != "2023 HT1*") %>%
...
B
ggsave("Plots/Poolseq infection level.png", plot = B ,width = 5, height = 5)


#### Histo ####

histo<-raw %>% group_by(Haplotype) %>% filter(!is.na(Haplotype)) %>%
  ggplot()+
...
ggsave("Plots/Pool Seq quantified samples distribution.png", plot = histo,width = 5, height = 5)
