...
ggsave(filename = "Plots/Pool seq result.png", plot = A, height = 5, width = 8)

##### Poster ####
pA <- 
  raw2 %>% filter(Haplotype != "HT3") %>% group_by(Haplotype) %>% 
  ggplot(aes(x="", y=labels, group = `Suggest Result`))+
  geom_bar(aes(fill=`Suggest Result`), 
           stat = "identity") +
  ylim(0,1) +
  ...
  xlab("") +
  ylab("") +
  labs(fill = "Wolbachia strain")+
  labs(title = "Wolbachia strain of untreated samples")
